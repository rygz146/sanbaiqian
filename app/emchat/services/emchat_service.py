...
from ..utils.http_utils import http_get, http_post, file_upload, get_file_stream, http_put, http_delete
from ..utils.file_utils import write
...
class EMIMUsersService(object):
    ...
    def query_users(self, limit, cursor=None):
        """获取IM用户[批量]
        """

        url = ('/%s/%s/users?limit=%s&cursor=%s' % (self.org, self.app, limit, cursor))
        return http_get(url, self.auth)
